...
export default function AdminNavbar() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        // Try to find employee name by email since user.uid might not match employee doc id
        try {
  ...
  const navLinks = [
    ...
    { name: 'Report', href: '/admin/reports', icon: <BarChart3 size={22} /> },
  ];

  const NavContent = () => (
    <div className="flex flex-col h-full py-10 px-6">

      {/* BRANDING - MATCHING EMPLOYEE STYLE */}
          ...
          return (
            ...
            >
              ...
              {isActive ? (
                ...
                link.alert && (
                  link.alertType === 'conflict' ? <Activity size={14} className="text-red-400" /> : <div className="w-2 h-2 rounded-full bg-orange-400" />
                )
              )}
            </Link>
          );
  ...
  return (
    <>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 h-24 bg-white/80 backdrop-blur-xl border-b border-slate-100 z-[60] flex items-center justify-between px-8">
        <p className="font-black text-lg tracking-tighter text-slate-900 uppercase italic leading-none">
          {userName} <br/><span className="text-slate-300 text-[10px]">Clear View LLC</span>
        </p>
      ...
      <div className={`lg:hidden fixed inset-0 z-[55] transition-all duration-500 ${isOpen ? 'visible' : 'invisible'}`}>
        ...
        />
        <aside className={`absolute left-0 top-0 bottom-0 w-[85%] max-w-sm bg-white shadow-2xl transition-transform duration-500 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
