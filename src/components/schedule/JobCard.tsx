...
import {
  MapPin, Plus, Minus, Clock, Banknote, CheckSquare, Mail, Lock, Unlock, ArrowRight, X,
  UserPlus, UserCheck, Activity, Smartphone, Sparkles, Waves, Droplets, Trash2, Calendar, HandCoins,
  Building2, ShieldCheck, ChevronDown, ChevronUp, Wind, RefreshCw, Layers, Send, Check, Loader2, Info, Users, Sun, Edit2
} from 'lucide-react';
...
const JobCard: React.FC<JobCardProps> = ({
  ...
  const [isOtherReferral, setIsOtherReferral] = useState(!!job.referralSource);
  ...
  useEffect(() => { setIsOtherReferral(!!job.referralSource); }, [job.referralSource]);
  useEffect(() => { 
  ...
  // Helper to remove 'undefined' fields that crash Firestore
  ...
  const generateJobHtml = (type: 'confirmation' | 'invoice', stripeUrl?: string) => {
    const title = type === 'invoice' ? 'Service Invoice' : 'Reservation Confirmed';
    const subtotal = mStats.lineItems
    ...
    const paymentBtn = (type === 'invoice' && stripeUrl) ? `
      <div style="margin-top: 30px; text-align: center;">
        <a href="${stripeUrl}" style="background-color: #0284c7; color: white; padding: 18px 40px; text-decoration: none; border-radius: 12px; font-weight: 800; text-transform: uppercase; font-size: 14px; display: inline-block;">Pay Invoice Securely</a>
      </div>` : '';
    ...
    <div style="max-width: 600px; margin: 0 auto; background: #ffffff; border-radius: 32px; overflow: hidden; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.1);">
        ...
        <div style="padding: 50px 40px;">
            <p style="font-size: 20px; line-height: 1.5; margin-bottom: 40px; color: #334155; font-weight: 300;">
                Hi <strong style="font-weight: 800; color: #0f172a;">${job.firstName}</strong>, ${type === 'invoice' ? 'here is your service invoice' : 'your reservation is confirmed! Our Team will be'} at <span style="color: #0284c7; font-weight: 600;">${fullAddress}</span>.
            </p>
            <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 40px; border-collapse: separate; border-spacing: 0 10px;">
                <tr>
                    <td style="background: #f8fafc; padding: 20px; border-radius: 16px 0 0 16px; border: 1px solid #e2e8f0; border-right: none;">
                        ...
                        <strong style="color: #0f172a; font-size: 16px;">${dateStr}</strong>
                    </td>
                    <td style="background: #f8fafc; padding: 20px; border-radius: 0 16px 16px 0; border: 1px solid #e2e8f0; border-left: none;">
                        ...
                        <strong style="color: #0284c7; font-size: 16px;">${arrivalTime}</strong>
                    </td>
                </tr>
            </table>
...
${job.selectedServices?.join('\n') || 'Services'}
            ...
            <div style="background: #0f172a; border-radius: 24px; padding: 35px; color: #ffffff;">
                <table width="100%" style="border-collapse: collapse;">
                    <tr>
                        ...
                        <td style="padding-bottom: 15px; text-align: right; font-weight: 600;">$${subtotal.toFixed(2)}</td>
                    </tr>
                    ${discount > 0 ? `<tr>
                        <td style="padding-bottom: 15px; text-align: right; color: #10b981; font-weight: 700;">-$${discount.toFixed(2)}</td>
                    </tr>` : ''}
                    <tr>
                        <td style="padding-top: 20px; border-top: 1px solid #1e293b; font-size: 16px; font-weight: 800;">Total Balance Due</td>
                        <td style="padding-top: 20px; border-top: 1px solid #1e293b; text-align: right; font-size: 28px; font-weight: 900; color: #38bdf8;">$${mStats.total}</td>
                    </tr>
                </table>
        ...
        <div style="background: #f8fafc; padding: 40px; border-top: 1px solid #f1f5f9;">
            <div style="text-align: center; margin-bottom: 30px;">
                <h4 style="margin: 0; text-transform: uppercase; font-size: 10px; letter-spacing: 4px; color: #94a3b8; font-weight: 800;">Preparation Guide</h4>
            </div>
  ...
  const sendInvoiceEmail = async () => {
    ...
    try {
      const response = await fetch('/api/stripe/create-checkout-session', {
        ...
        body: JSON.stringify({ amount: parseFloat(mStats.total), leadId: job.id, customerEmail: finalEmail, customerName: `${job.firstName} ${job.lastName}` })
      });

      const data = await response.json();
      if (data.url) {
        const html = generateJobHtml('invoice', data.url);
        // 1. Direct write to 'mail' (Matching working QuoteModal flow)
        await addDoc(collection(db, "mail"), { 
          to: [finalEmail, 'clearview3cleaners@gmail.com'], 
          message: { subject: `Service Invoice - Clear View LLC`, html: html } 
        });
        ...
        throw new Error(data.error || "Stripe Link Error");
      }
  ...
  const resendConfirmation = async () => {
    ...
    try {
      const html = generateJobHtml('confirmation');
      // 1. Direct write to 'mail' (Matching working QuoteModal flow)
      await addDoc(collection(db, "mail"), { 
        to: [finalEmail, 'clearview3cleaners@gmail.com'], 
        message: { subject: `Reservation Confirmed - Clear View LLC`, html: html } 
      });
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-6 bg-slate-50/50 p-6 rounded-[2rem] border border-slate-100">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      ...
                      <SubToggle label="Downspout Flush" active={job.gutterFlush} onClick={() => updateJob?.(job.id, job, { gutterFlush: !job.gutterFlush })} icon={<Droplets size={12}/>} iconSize={12}/>
                      <SubToggle label="Roof Blow-off" active={job.roofBlowOff} onClick={() => updateJob?.(job.id, job, { roofBlowOff: !job.roofBlowOff })} icon={<Wind size={12}/>} iconSize={12}/>
                      <SubToggle label="Baking Soda Treatment" active={job.mossTreatment} onClick={() => updateJob?.(job.id, job, { mossTreatment: !job.mossTreatment })} icon={<Activity size={12}/>} iconSize={12}/>
                      <SubToggle label="Acid Wash Removal" active={job.mossAcidWash} onClick={() => updateJob?.(job.id, job, { mossAcidWash: !job.mossAcidWash })} icon={<Sparkles size={12}/>} iconSize={12}/>
                    </div>
                  </div>
                  <div className="space-y-6 bg-slate-50/50 p-6 rounded-[2rem] border border-slate-100">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      <SubToggle label="Deluxe Screen Detail" active={job.deluxeWindow} onClick={() => updateJob?.(job.id, job, { deluxeWindow: !job.deluxeWindow })} icon={<Sun size={12}/>} iconSize={12}/>
                      <SubToggle label="Exterior Gutter Wash" active={job.deluxeGutter} onClick={() => updateJob?.(job.id, job, { deluxeGutter: !job.deluxeGutter })} icon={<Sparkles size={12}/>} iconSize={12}/>
                      <SubToggle label="Military Discount (10%)" active={job.militaryDiscount} onClick={() => updateJob?.(job.id, job, { militaryDiscount: !job.militaryDiscount })} icon={<ShieldCheck size={12}/>} iconSize={12}/>
                    </div>
                  </div>
                </div>
